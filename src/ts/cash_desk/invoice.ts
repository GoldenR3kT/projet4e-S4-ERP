document.addEventListener('DOMContentLoaded', function() {
    ...
    function clearSelections(items: any[] | NodeListOf<Element>) {
        items.forEach(function(item) {
            item.classList.remove('selected');
        });
    }

    function clearOtherSelections(items: any[] | NodeListOf<Element>) {
        items.forEach(function(item) {
            item.classList.remove('selected');
        });
    }

    function updateSelectionText(text: string | null) {
        // @ts-ignore
